...
const OptionalService = () => {
  ...
  <div className="bg-white rounded-lg p-8 shadow-xl text-center">
    <h1 className="text-3xl font-semibold mb-4">{t("Optional Services")}</h1>
    <div className="space-y-4">
      {optionalServices && typeof optionalServices === 'object' && Object.keys(optionalServices).length === 0 ? (
        <p>{t("No optional services available")}.</p>
      ) : optionalServices ? (
        Object.values(optionalServices).map(service => (
          <div key={service.ratesID} className="p-6 rounded-md shadow-md text-left">
            <span className="font-semibold p-1">{t("Rates ID")}:</span> {service.ratesID}<br />
            <span className="font-semibold p-1">{t("Rates Name")}:</span> {service.ratesName}
          </div>
        ))
      ) : (
        <p className='font-poppins'>{t("Loading optional services")}...</p>
      )}
    </div>
